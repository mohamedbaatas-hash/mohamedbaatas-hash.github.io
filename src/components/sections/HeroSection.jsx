...
export function HeroSection() {
  ...
  const heroData = t('HERO_DATA', { returnObjects: true });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded bg-cobalt-950/40 border border-cobalt-500/30 text-cobalt-300 font-mono text-xs">
            ...
            <span>{t('HERO.tag', 'Hydraulics + IoT Systems')}</span>
          </div>
        </div>
        ...
        <div className="max-w-4xl">
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-white leading-tight">
            {t('HERO.title1', 'Engineering High-Precision')} <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cobalt-400 via-cyan-300 to-blue-200">
              {t('HERO.title2', 'Hydroinformatics & SCADA')}
            </span> {t('HERO.title3', 'Telemetry')}
          </h1>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-4">
          ...
          >
            ...
            <span>{t('HERO.download_cv', 'Download Resume / CV')}</span>
          </a>
          <a
            href="#case-studies"
          ...
          >
            <span>{t('HERO.explore', 'Explore Engineering Case Studies')}</span>
          ...
          >
            ...
            <span>{t('HERO.scada', 'SCADA Simulator')}</span>
          </a>
          <a
            href="#epanet-sim"
            className="inline-flex items-center gap-2 px-5 py-3 rounded-lg bg-elevated-slate hover:bg-slate-700 text-slate-200 border border-border-slate font-mono text-sm transition-colors"
          >
            ...
            <span>{t('HERO.epanet', 'EPANET Solver')}</span>
          </a>
        </div>

        {/* High-Density KPI Strip */}
        <div className="mt-14 grid grid-cols-2 md:grid-cols-4 gap-4">
          {heroData.kpis && heroData.kpis.map((kpi, idx) => (
            <MetricBadge
              key={idx}
              label={kpi.label}
              value={kpi.value}
